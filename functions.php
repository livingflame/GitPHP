 ...
 */
	...
	function getBaseUrl($replace = "index.php"){
		...
		return isset($replace) ? str_replace($replace, NULL, $domain) : $domain;
	}
	...
	function siteUrl($file=""){
		...
		$port = (isset($_SERVER['SERVER_PORT']) && ((!$isHTTPS && $_SERVER['SERVER_PORT'] != "80") || ($isHTTPS && $_SERVER['SERVER_PORT'] != "443")));
		$port = ($port) ? ':'.$_SERVER['SERVER_PORT'] : '';
		$siteURL = ($isHTTPS ? 'https://' : 'http://').$_SERVER['SERVER_NAME'].$port.$requri . $file;
		return $siteURL;
	}
    ...
    function debugTraceAsString($trace_array, $ignore = 0 ){
        ...
        foreach($trace_array as $key => $stackPoint){
            ...
            if($stackclass){
                ...
                $error_arg .= '</ol>';
				$trace .= '<div style="background-color: #d6ffef; border: 1px solid #bbb; border-radius: 4px; -moz-border-radius: 4px; -webkit-border-radius: 4px; font-size: 12px; line-height: 1.4em; margin: 8px 0; padding: 8px 8px 0 8px; overflow: auto"><h2 style="font-size: 18px; margin: 0; padding: 0 0 8px 0">'. $c . ". " . $stackfile . $stackline . " &raquo; $" . $stackclass . $stacktype . $stackfunc . '()</h2>';
                if($count){
                    $trace .= "<pre style='margin: 0px 0px 10px 0px; display: block; background: white; color: black; font-family: Verdana; border: 1px solid #cccccc; padding: 5px; font-size: 10px; line-height: 13px;'>";
                    $trace .= $error_arg;
                    $trace .= "</pre>";
                }
	...
	function dump($var, $info = FALSE)
	{
	    ...
	    echo "</pre>";
        echo debugTraceAsString(debug_backtrace(false));
	}
    ...
    function template( $names, $args ){
        ...
        foreach ( $names as $name ) {
            $file = realpath(dirname(__FILE__)) . '/templates/' . $name . '.tpl';
    ...
    function path($file){
        ...
        $root = unifiedPath(GITPHP_BASEDIR);
    ...
    function sluggedFilename($file){
        ...
        $path_parts['filename'] = slugify($path_parts['filename']);
    ...
    function tmce($text){
        $allowedTags='<p><strong><em><u><h1><h2><h3><h4><h5><h6><img><object>';
        $allowedTags.='<li><ol><ul><span><div><br><ins><del>';
        return strip_tags(stripslashes(htmlspecialchars_decode($text)),$allowedTags);;
    }

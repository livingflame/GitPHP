...
class GitPHP_Project
{
	 ...
	 */
	public function __construct($config, $project, GitPHP_ProjectLoadStrategy_Interface $strategy = null)
	{
        $this->config = $config;
		$this->projectRoot = GitPHP_Util::AddSlash($config->GetValue('projectroot'));
		...
		if (is_file(GitPHP_Util::AddSlash($this->GetPath()) . '.repopickle_config')) {
			...
			$this->showRemotes = $this->config->GetValue('showremotes', false);
		}
	}
	...
	public function GetBugUrl()
	{
		if ($this->bugUrl != null)
			return $this->bugUrl;
        return $this->config->GetValue('bugurl', '');
	}
	...
	public function GetBugPattern()
	{
        ...
        return $this->config->GetValue('bugpattern', '');
	 ...
	 * @author tpruvot
	 *
	 * @param GitPHP_RemoteHeadList $remoteList head list
	 */
	public function SetRemoteList($remoteList)
	{
		if ($remoteList && ($remoteList->GetProject() !== $this))
			throw new Exception('Invalid remote remotelist for this project');
	...
	public function GetRemotes($count = 0)
	{
		...
		return $this->GetRemoteList()->GetOrderedRemotes('-committerdate', $count);
	}
}

...
class GitPHP_ProjectLoad_Git implements GitPHP_ProjectLoadStrategy_Interface
{
	...
	public function LoadHead($project)
	{
		...
		$args[] = '--verify';
		$args[] = 'HEAD';
		return trim(implode("\n",$this->exe->Execute($project->GetPath(), GIT_REV_PARSE, $args)));
	}
	...
	public function ExpandHash($project, $abbrevHash)
	{
		...
		$fullData = $this->exe->Execute($project->GetPath(), GIT_REV_LIST, $args);
		if (empty($fullData[0])) {
			return $abbrevHash;
		}
	...
	public function AbbreviateHash($project, $hash)
	{
		...
		$abbrevData = $this->exe->Execute($project->GetPath(), GIT_REV_LIST, $args);

   ...
   {foreach from=$revlist item=rev}
	 ...
	 {include file='refbadges.tpl' commit=$rev}
       ...
       <td class="link">
         ...
         <a href="{geturl project=$project action=commit hash=$rev}">{t}commit{/t}</a> | <a href="{geturl project=$project action=commitdiff hash=$rev}">{t}commitdiff{/t}</a> | <a href="{geturl project=$project action=tree hash=$revtree hashbase=$rev}">{t}tree@{$rev->GetHash(true)}{/t}</a> | <a href="{geturl project=$project action=snapshot hash=$rev}" class="snapshotTip">{t}snapshot{/t}</a>
	 {if $source == 'shortlog'}
	  | 
	  {if $mark}
